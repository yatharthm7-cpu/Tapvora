...
type WizardBusiness = { id: string; name: string; reviewUrl: string; address: string };
...
export function CardSetupWizard({ cards, businesses }: { cards: WizardCard[]; businesses: WizardBusiness[] }) {
  ...
  return <form className="wizard" action={setupCardAction}>
    ...
    <section className="wizard-step"><span>2</span><div><div className="wizard-title"><Store size={18} /><h2>Select or add business</h2></div><p>Use a saved customer or create a new customer profile.</p><select className="select" name="business_id" value={businessId} onChange={(event) => chooseBusiness(event.target.value)}><option value="new">+ Add a new business</option>{businesses.map((business) => <option key={business.id} value={business.id}>{business.name}</option>)}</select>{businessId === "new" ? <input className="input wizard-extra" name="business_name" value={businessName} onChange={(event) => setBusinessName(event.target.value)} placeholder="Business name" required /> : null}<textarea className="textarea wizard-extra" name="address" value={address} onChange={(event) => setAddress(event.target.value)} rows={2} maxLength={500} autoComplete="street-address" placeholder="Business address (optional)" /></div></section>
