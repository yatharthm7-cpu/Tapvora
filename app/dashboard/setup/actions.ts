...
export async function setupCardAction(formData: FormData) {
  ...
  if (selectedBusinessId === "new") {
    ...
    const { data, error } = await supabase.from("businesses").insert({ name: newBusinessName, review_url: reviewUrl }).select("id").single();
    ...
    const { error: businessUpdateError } = await supabase.from("businesses").update({ review_url: reviewUrl }).eq("id", business.id);
